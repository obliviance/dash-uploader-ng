"""VENDORED, UNMODIFIED: upstream dash-uploader's request handler.

Source:  https://github.com/fohrloop/dash-uploader
Commit:  e1a1af47aa607b7aa3deceafe5c5452d98465610 (the last commit before the
         repository was archived on 2025-07-19)
File:    dash_uploader/httprequesthandler.py
Licence: MIT, (c) Niko Föhr -- see LICENSE.txt at the repository root.

This is the *vulnerable* code, kept here on purpose: tests/test_upstream_parity.py
runs it side by side with this fork's handler so that "the fork still behaves
like dash-uploader" and "the fork no longer has CVE-2026-38360" are both
demonstrated against the real thing rather than against a paraphrase of it.

The ONLY edit is the `from dash_uploader.utils import retry` import, repointed
at `dash_uploader_ng.utils` so the module is importable here. `retry` is
unchanged between the two, so this does not affect behaviour.

Do not "fix" anything in this file. Its whole value is being unpatched.
"""
